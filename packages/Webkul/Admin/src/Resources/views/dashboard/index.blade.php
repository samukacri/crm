<x-admin::layouts>
    ...
    @pushOnce('scripts')
        ...
        <script
            type="module"
            src="{{ asset('admin/build/assets/app-CpfUy-4D.js') }}"
        >
        </script>

        <script type="module">
            // Carregar chartjs-chart-funnel de forma assíncrona após o Chart.js estar disponível
            import('https://cdn.jsdelivr.net/npm/chartjs-chart-funnel@4.2.1/build/index.umd.min.js')
                .then(module => {
                    // Registrar o funnel chart globalmente
                    if (typeof window.Chart !== 'undefined' && module && module.default) {
                        window.Chart.register(module.default);
                    }
                })
                .catch(error => {
                    console.error('Erro ao carregar chartjs-chart-funnel:', error);
                });
        </script>
